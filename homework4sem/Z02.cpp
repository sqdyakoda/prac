    ...
    Token(Type kind, std::string str, int l, int c) : type(kind), value(str), line(l), column(c) {
        if (type == CONSTANTS) {
            if ((str != "true") && (str != "false") && (str != "null")) {
				std::cout << "Syntax error: (" << line << ", " << column << ") " << value;
...
public:
    ...
    Token getToken() {
        ...
        if (state == S)
            return Token(END, str, line + 1, 1);
        //std::cout << "(" << line << ", " << column << ") ERROR : " << str << std::endl;
        std::cout << "Syntax error: (" << line << ", " << column << ") " << str << std::endl;
        exit(0);
    }
};
    ...
    bool parse() {
        try {
			...
			if (t.getKind() != END)
				...
				std::cout << "EOF";
			*/
			std::cout << "Syntax error: (" << t.getLine() << ", " << t.getColumn() << ") ";
			if (t.getKind() != END)
				std::cout << t.getValue();
			else
				std::cout << "EOF";
